...
(define (parse args build-args flash-args)
  (cond
    ...
    ((or (string=? (car args) "--wifi-firmware")
         ...
         (string=? (car args) "--uart-pin-probe"))
...
(let ((args (command-line-tail)))
  (when (and (pair? args) (string=? (car args) "--help"))
    (say "usage: tools/build-flash-lisp.scm [--wifi-firmware] [--wifi-credentials] [--wifi-boot-smoke] [--wifi-dhcp-boot-smoke] [--wifi-arp-boot-smoke] [--wifi-dns-boot-smoke] [--wifi-net-repl-boot-smoke] [--wifi-net-repl-service-boot-smoke] [--storage-boot-smoke] [--storage-format-boot-smoke] [--skip-boot-file] [--uart-pin-probe] [flash-lisp arguments]")
    (say "")
